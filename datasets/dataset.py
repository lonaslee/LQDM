...
class Dataset(torch.utils.data.Dataset):
            ...
            if self.labels_by_id:
                ...
                if check_empty_targets:
                    with target_zip.open(target_filename) as target_file:
                        min_val, max_val = Image.open(target_file).convert("L").getextrema()
                        if min_val == max_val:
                            continue
    ...
    def __getitem__(self, index: int):
        ...
        for m in masks[1:]:
            mask = m.cpu().numpy().astype("uint8")
            contours, _ = cv2.findContours(mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
            boundary_mask = np.zeros_like(mask)

            diagonal = np.sqrt(mask.shape[0] ** 2 + mask.shape[1] ** 2)
            thickness = max(1, int(diagonal * 0.01))

            boundary_mask = cv2.drawContours(boundary_mask, contours, -1, 1, thickness)
            boundary = torch.from_numpy(boundary_mask).long()
            boundaries.append(boundary)
            combined = torch.logical_or(combined, boundary)
        boundaries[0] = 1 - combined.long()

        target = {
            "masks": tv_tensors.Mask(torch.stack(masks)),
            "boundaries": tv_tensors.Mask(torch.stack(boundaries)),
    ...
    @staticmethod
    def _get_boundary(mask: torch.Tensor, thickness: int = 8) -> torch.Tensor:
        tmp = mask.cpu().numpy().astype("uint8")
        contours, _ = cv2.findContours(tmp, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
        boundary = np.zeros_like(tmp)
        boundary = cv2.drawContours(boundary, contours, -1, 1, thickness)
