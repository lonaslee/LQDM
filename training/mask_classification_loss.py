...
class BoundaryBCELoss(nn.Module):
    def __init__(self, boundary_coefficient: float = 2.0):
        super().__init__()
    ...
    def forward(
            self,
            boundary_queries_logits: torch.Tensor,
            targets: list,
    ):
        batch_size = boundary_queries_logits.shape[0]
        device = boundary_queries_logits.device

        target_masks = []
        logit_h, logit_w = boundary_queries_logits.shape[2:]  # 128, 128

        for b in range(batch_size):
            batch_targets = targets[b]["masks"]  # [Q, 512, 512]

            if batch_targets.shape[0] > 0:
                # [Q, 512, 512] -> [Q, 128, 128]
                resized_masks = F.interpolate(
                    batch_targets.unsqueeze(1).float(),  # [Q, 1, 512, 512]
                    size=(logit_h, logit_w),
                    mode='bilinear',
                    align_corners=False
                ).squeeze(1)  # [Q, 128, 128]

                num_queries = boundary_queries_logits.shape[1]
                if resized_masks.shape[0] < num_queries:
                    padding = torch.zeros(num_queries - resized_masks.shape[0], logit_h, logit_w, device=device)
                    resized_masks = torch.cat([resized_masks, padding], dim=0)
                else:
                    resized_masks = resized_masks[:num_queries]

                target_masks.append(resized_masks)
            else:
                target_masks.append(torch.zeros(boundary_queries_logits.shape[1], logit_h, logit_w, device=device))

        target_masks = torch.stack(target_masks)  # [B, Q, H, W]

        bce_loss = self.bce_loss(boundary_queries_logits, target_masks.float())

        return {
            "loss_boundary": bce_loss * self.boundary_coefficient
        }
