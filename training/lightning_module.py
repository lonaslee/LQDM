...
from torchmetrics.classification import MulticlassJaccardIndex, MulticlassAccuracy, MulticlassF1Score, MulticlassRecall, MulticlassPrecision
...
class LightningModule(lightning.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.network.predict_boundaries:
            mask_logits_per_block, class_logits_per_block, boundary_logits_per_block = self(imgs)
        else:
            mask_logits_per_block, class_logits_per_block = self(imgs)
            boundary_logits_per_block = [None for _ in range(len(mask_logits_per_block))]

        losses_all_blocks = {}
        for i, (mask_logits, class_logits, boundary_logits) in enumerate(
                list(zip(mask_logits_per_block, class_logits_per_block, boundary_logits_per_block))
        ):
            ...
            if self.network.predict_boundaries:
                boundary_targets = []
                for t in targets:
                    device = t["boundaries"].device
                    is_crowd = t["is_crowd"]

                    # Ensure is_crowd is on the correct device
                    if isinstance(is_crowd, torch.Tensor):
                        is_crowd = is_crowd.to(device)
                    else:
                        is_crowd = torch.tensor(is_crowd, device=device)

                    boundary_target = {
                        "masks": t["boundaries"],
                        "labels": torch.zeros(t["boundaries"].shape[0], dtype=torch.long, device=device),
                        "is_crowd": is_crowd,
                    }
                    boundary_targets.append(boundary_target)

                boundary_losses = self.criterion2(boundary_logits, boundary_targets)
    ...
    def init_metrics_semantic(self, ignore_idx, num_blocks):
        print(f"Init metrics: ignore_idx={ignore_idx}, num_blocks={num_blocks}")
        self.metrics = nn.ModuleList([MetricCollection({
            "iou": MulticlassJaccardIndex(num_classes=self.num_classes, ignore_index=ignore_idx, average=None),
            "acc": MulticlassAccuracy(num_classes=self.num_classes, ignore_index=ignore_idx, average=None),
            "pre": MulticlassPrecision(num_classes=self.num_classes, ignore_index=ignore_idx, average=None),
            "rec": MulticlassRecall(num_classes=self.num_classes, ignore_index=ignore_idx, average=None),
            "f_1": MulticlassF1Score(num_classes=self.num_classes, ignore_index=ignore_idx, average=None),
            # "exm": MulticlassExactMatch(num_classes=self.num_classes, ignore_index=ignore_idx, multidim_average="global"),
        }) for _ in range(num_blocks)])

    def init_metrics_instance(self, num_blocks):
        self.metrics_iou = nn.ModuleList(
    ...
    @torch.compiler.disable
    def update_metrics_instance(self, preds: list[dict], targets: list[dict], block_idx):
        self.metrics_iou[block_idx].update(preds, targets)

    @torch.compiler.disable
    def update_metrics_panoptic(self, preds: list[torch.Tensor], targets: list[torch.Tensor],
                                is_crowds: list[torch.Tensor], block_idx):
        for i in range(len(preds)):
            metric = self.metrics_iou[block_idx]
    ...
    def _on_eval_epoch_end_semantic(self, log_prefix, log_per_class=False):
        for i, metric_collection in enumerate(self.metrics):
            block_postfix = self.block_postfix(i)
            if block_postfix != "": continue

            metric_collection: MetricCollection
            computed_metrics = metric_collection.compute()
            for name, computed in computed_metrics.items():
                computed: torch.Tensor
                self.log(f"metrics/{log_prefix}_{name}_all", computed.mean().float())
                if log_per_class:
                    for clsidx, clsval in enumerate(computed):
                        self.log(f"metrics/{log_prefix}_{name}_class_{clsidx}", clsval)

    def _on_eval_epoch_end_instance(self, log_prefix):
        for i, metric in enumerate(self.metrics_iou):  # type: ignore
